...
- **Dropdown/Select/Combobox — ต้องมี "re-open after selection":** ทุก test plan ที่ครอบคลุม dropdown, select, combobox, หรือ searchable list ต้องมี test case สำหรับ flow นี้เสมอ: (1) เปิด dropdown → เลือก item → (2) คลิกเปิด dropdown อีกครั้ง → ต้องเห็น option ครบทุกรายการ (ไม่ถูก filter จากค่าที่เลือกก่อนหน้า)
