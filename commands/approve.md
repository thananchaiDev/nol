...
#### Step 4.5: Visual Check (บังคับเมื่อมี frontend changes)

ตรวจว่า implementation มี frontend changes หรือไม่ — ไฟล์ประเภทใดก็ได้ที่ render ใน browser: `.vue`, `.jsx`, `.tsx`, `.html`, `.css`, `.scss`, template, component

**ถ้ามี frontend changes → ต้องทำ visual check ทุกครั้ง ห้ามข้าม:**

1. เปิด browser ด้วย `mcp__chrome-devtools__navigate_page` ไปยัง URL ของหน้าที่เปลี่ยน
2. ใช้ `mcp__chrome-devtools__take_screenshot` ถ่ายภาพหน้าจอ
3. ตรวจสอบ **ทุกข้อ** ต่อไปนี้กับสิ่งที่ SOLUTION.md ระบุว่าต้องเปลี่ยน:
   - UI element ที่ควรแสดงผล → แสดงถูกต้องไหม?
   - ข้อความ / label / wording → ตรงกับ requirement ไหม?
   - Layout / spacing / สี → ผิดเพี้ยนจาก design ไหม?
   - ถ้ามี interaction (click, input) → ทำงานถูกต้องไหม?
4. ตรวจ console errors ด้วย `mcp__chrome-devtools__list_console_messages`
5. บันทึกผล visual check พร้อม screenshot path ลง test results

**ถ้า visual check พบปัญหา → วน loop แก้ไขเหมือน test ปกติ ห้ามผ่านโดยไม่แก้**
...
- **Frontend changes → Visual check บังคับ** — lint + build ผ่านไม่พอ ต้องเปิด browser ดูจริงทุกครั้ง
