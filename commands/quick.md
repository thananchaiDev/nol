...
### Step 4.5: Launch Learn Agent

Launch **1 agent** กับ `run_in_background: true`:

| Agent | subagent_type | Prompt Variables |
|-------|--------------|------------------|
| Learn | `learn` | `TASK_DESCRIPTION` = task description, `CURRENT_DIR` = `{QUICK_DIR}`, `REFERENCE_LABEL` = `{REFERENCED_LABEL}` หรือ `"quick {number}"` ถ้า REFERENCED_LABEL ว่าง, `REFERENCE_SUMMARY_PATH` = `{REFERENCE_SUMMARY_PATH}` หรือ `"{QUICK_DIR}/SUMMARY.md"` ถ้า REFERENCED_LABEL ว่าง, `MISTAKE_DIR` = `.nol/mistake/` |
...
Step 4:  Write SUMMARY.md (header + links)
         ↓
Step 4.5 (1 background, always):  learn agent → .nol/mistake/YYYY-MM-DD.md
         ↓ (ไม่รอ)
Step 5:  /nol:recap quick {number}  →  feedback loop (handled by recap)
```

**Total agents: 6** (1 + 1 + 3 + learn agent เสมอ)
