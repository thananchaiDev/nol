...
- Format: `feature <query>`, `bugfix <query>`, หรือพิมพ์แค่ type เดียว/ไม่พิมพ์อะไรเลย
- `<query>` อาจเป็นตัวเลข (เช่น `1`), ชื่อเต็ม (เช่น `add-hscode-sorting`), หรือคำบางส่วน (เช่น `hscode`, `login`)

---

### Case 1: ระบุ type และ query (เช่น `/recap feature 1`, `/recap feature hscode`)
...
3. อ่านไฟล์ plan ทั้งหมดพร้อมกัน: `FEATURE.md` หรือ `BUG.md`, `IMPACT.md`, `SOLUTION.md`
...
5. **Present ให้ user** ในรูปแบบนี้:
   ...
   ```
   ## 📋 Recap: [ชื่อ feature/bugfix]

   **Type:** Feature / Bugfix
   **Folder:** .nol/feature/1-xxx/

   ### สรุป
   [เนื้อหาจาก FEATURE.md หรือ BUG.md — เขียนให้กระชับ ไม่ใช่แค่ copy paste]

   ### Implementation Status — [X/Y ไฟล์ · Z%]

   | ไฟล์ | สถานะ | หมายเหตุ |
   |------|--------|---------|
   | `sdk/schema/db/sbsc_api/SYS_Hscode.js` | ✅ Done | เพิ่ม Sort field + IDX_Sort แล้ว |
   | `sdk/Service.js` | ❌ Not yet | ยังไม่มี order clause ใน getHscode() |
   | `sdk/Inter/InterShipment.js` | ⚠️ Partial | มี order clause แต่ยังไม่มี require('sequelize') |
...
6. **Feedback Loop — ถามทันทีหลัง present และวนไม่มีที่สิ้นสุดจนกว่า user จะบอกว่าไม่มี feedback แล้ว:**
   ...
   **ถ้า user ให้ feedback หรือขอปรับแก้แผน:**
   1. ระบุว่า feedback นั้นกระทบไฟล์ไหน:
      - คำอธิบาย bug/feature → แก้ `BUG.md` หรือ `FEATURE.md`
...
1. ค้นหา directories ทั้งหมดใน path ที่เกี่ยวข้อง:
   ...
   - ไม่ระบุ → ทั้ง `.nol/feature/` และ `.nol/bugfix/`

2. สำหรับแต่ละ directory ให้แสดง:
   - เลขและชื่อ
   - มี SUMMARY.md หรือไม่
   - ถ้ามี → อ่าน 1-2 บรรทัดแรกของ SUMMARY.md มาแสดงเป็น preview
...
3. **Present รายการ** ในรูปแบบนี้:
   ...
   พิมพ์ `/recap feature 1` เพื่อดูรายละเอียดและ implementation status
   ```
...
- ใช้ `FEATURE.md`/`BUG.md` เป็นแหล่งสรุปเนื้อหา (ไม่ใช่ SUMMARY.md)
- สรุปให้กระชับ อ่านง่าย ไม่ต้อง copy paste ทุกบรรทัด
- ใช้ภาษาเดียวกับที่ user ใช้ใน description ของ feature/bugfix นั้น
- ถ้าไม่มี directory `.nol/` เลย ให้แจ้ง user ว่ายังไม่มี feature หรือ bugfix ใดถูก plan ไว้
